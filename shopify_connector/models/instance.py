...
class ShopifyInstance(models.Model):
    ...
    def _clear_managed_access_token(self, values):
        ...
        Shopify call, with the credentials that were just stored. Values written
        along with the credentials are left alone, as they are the new ones.
        """
        cleared = {
            name: False
            for name in ("access_token", "access_token_expires_at")
